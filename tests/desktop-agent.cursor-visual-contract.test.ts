...
describe("Desktop Agent Cursor-style sidebar visual contract", () => {
  it("keeps user prompts as full-width rows and Agent replies in document flow", () => {
    ...
    expect(css).toMatch(/\.desktop-agent-turn-summary\s*\{[^}]*padding-inline:\s*var\(--agent-message-content-inset\)[^}]*color:\s*var\(--agent-text-subtle\)[^}]*font-size:\s*var\(--agent-font-size\)[^}]*line-height:\s*1\.66[^}]*text-align:\s*left/s);
  ...
  it("keeps chrome in flow and gives the composer explicit control sizing", () => {
    ...
    expect(css).toMatch(/\.desktop-agent-composer-actions\s*\{[^}]*display:\s*flex[^}]*margin-left:\s*auto/s);
    ...
    expect(css).toMatch(/\.desktop-agent-composer textarea\s*\{[^}]*min-height:\s*var\(--agent-composer-text-min-height\)[^}]*max-height:\s*var\(--agent-composer-text-max-height\)[^}]*padding:\s*0[^}]*font-size:\s*var\(--agent-font-size\)[^}]*line-height:\s*var\(--agent-composer-line-height\)/s);
    ...
    expect(css).toMatch(/\.desktop-agent-session-menu\s*\{[^}]*right:\s*auto[^}]*left:\s*0/s);
    ...
    expect(css).toMatch(/\.desktop-agent-jump-latest\s*\{[^}]*position:\s*absolute[^}]*left:\s*50%[^}]*width:\s*var\(--agent-control-size\)[^}]*height:\s*var\(--agent-control-size\)[^}]*border-radius:\s*50%[^}]*transform:\s*translateX\(-50%\)/s);
  ...
  it("uses one compact highlighted row grammar for every native tool", () => {
    ...
    expect(css).toMatch(/\.desktop-agent-tool-branch\s*\{[^}]*border:\s*0[^}]*border-left:\s*2px solid var\(--agent-border-subtle\)[^}]*background:\s*transparent/s);
