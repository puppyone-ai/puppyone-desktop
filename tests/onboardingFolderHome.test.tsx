...
describe("project folder home", () => {
  ...
  it("uses one square primary action above two compact secondary actions", async () => {
    ...
    expect(css).toMatch(/\.onboarding-brand-lockup\s*\{[^}]*width:\s*min\(324px, 100%\);[^}]*flex-direction:\s*row;[^}]*align-items:\s*center;[^}]*justify-self:\s*center;[^}]*justify-content:\s*flex-start;[^}]*gap:\s*10px;/s);
    ...
    expect(css).toMatch(/\.onboarding-primary-area\s*\{[^}]*width:\s*min\(324px, 100%\);[^}]*justify-self:\s*center;[^}]*justify-items:\s*center;[^}]*padding:\s*0;[^}]*border:\s*0;[^}]*background:\s*transparent;/s);
    ...
    expect(css).toMatch(/\.onboarding-entry-action-primary\s*\{[^}]*border-color:\s*var\(--po-border-strong\);[^}]*background:\s*transparent;[^}]*color:\s*var\(--po-text\);[^}]*font-weight:\s*var\(--po-text-weight-semibold, 600\);/s);
    expect(css).toMatch(/\.onboarding-entry-action-folder\s*\{[^}]*width:\s*100%;[^}]*height:\s*auto;[^}]*min-height:\s*0;[^}]*aspect-ratio:\s*1;[^}]*flex-direction:\s*column;[^}]*justify-content:\s*center;[^}]*gap:\s*14px;[^}]*text-align:\s*center;/s);
