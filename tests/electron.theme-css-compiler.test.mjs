...
describe("CSS theme compiler", () => {
  ...
  it("scopes the documented dark theme-root forms without escaping the surface", async () => {
    ...
    expect(application.css).toBe(`${appHost}:where(.dark), ${appHost}:where(.dark) { --po-surface-canvas: #111827 }`);
    expect(markdown.css).toContain(`:where(.dark) ${markdownHost}`);
  });
  ...
  it("rejects !important in ordinary themes so later Editor and Custom layers remain authoritative", async () => {
    await expect(compileThemeCss({
      ...
      target: "markdown",
    })).rejects.toThrow("cannot use !important");
  });
});
