...
describe("lightweight Office preview experience", () => {
  ...
  it("presents PowerPoint as a thumbnail rail and one central slide stage", () => {
    ...
    // Base Light/Neutral, its explicit Dark variant, and every palette that
    // intentionally changes file accents. Light/Warm still inherits them.
    expect(paletteTokenSource.match(/--po-file-accent-presentation:/g)).toHaveLength(6);
    expect(paletteTokenSource.match(/--po-file-accent-word:/g)).toHaveLength(6);
