...
export function findPackCandidates(input: {
  ...
  const wantedMimeTypes = new Set(
    input.mimeTypes.map((item) => item.trim().toLowerCase()).filter(Boolean),
  );
  ...
  return contributions.filter((contribution) => {
    ...
    return contribution.formats.some((format) => {
      ...
      const mimeHit = format.mimeTypes.some((mime) =>
        wantedMimeTypes.has(mime.toLowerCase()),
      );
      return extensionHit || mimeHit;
    });
  });
}
...
export function resolveViewerRoute(input: ResolveViewerRouteInput): ViewerRouteResult {
  const capability = coreViewerCapability(input.coreViewerId);
