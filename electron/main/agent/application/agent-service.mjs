...
export function createAgentService({
  ...
  function getReferenceInputCapabilities(sender, sessionId, workspaceRoot = null) {
    const session = requireOwnedSession(sender, normalizeRequiredId(sessionId, "Agent session id"));
    requireMatchingWorkspace(session, workspaceRoot);
    return session.capabilities?.referenceInputs
      ? { ...session.capabilities.referenceInputs }
      : null;
  }
