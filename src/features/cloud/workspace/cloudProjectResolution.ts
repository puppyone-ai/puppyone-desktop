...
export async function resolveRecentWorkspaceCloudBinding({
  ...
  const config = await readPuppyoneWorkspaceConfig(item.workspace.path).catch(() => null);
  const projectId = config?.cloud.projectId?.trim() || null;
  const bindingId = config?.cloud.bindingId?.trim() || null;
  const origin = config?.cloud.origin?.trim() || null;
  const configuredInstance = config?.project.workspaceInstanceId?.trim() || null;
  ...
  const gitStatus = await getWorkspaceGitStatus(item.workspace.path).catch(() => null);
  if (getPuppyoneRemote(gitStatus)) {
    return [item.workspace.id, {
      projectId: null,
      cloudLinked: true,
