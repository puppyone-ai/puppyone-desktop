...
export function resolveProjectCloudAttachment({
  ...
  if (projectId) {
    const source = resolutionSource ?? "workspace-binding";
    const durableStatus = bindingStatus
      ?? (source === "workspace-binding" ? "bound" : "not-bound");
    const bindingDetails = {
      resolutionSource: source,
      bindingStatus: durableStatus,
      ...(bindingId ? { bindingId } : {}),
...
export function resolveCloudProjectNavigationContext(
  ...
  // A transient catalog selection is never allowed to decide the context of
  // an already-open Local workspace. Global browsing owns a separate route.
  void selectedProjectId;
  return {
    projectContext,
    projectBound: projectContext,
  };
}
