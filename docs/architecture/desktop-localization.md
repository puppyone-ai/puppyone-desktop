...
| Semantic boundaries | Cloud workspace binding, project/backup status, Agent lifecycle errors, document-session errors, Automation definitions, and other first-party state cross boundaries as stable codes plus values. Raw provider or diagnostic detail is not used as message selection. |
