...
Status: implemented canonical contract. The acquisition, staging, capability,
submission and transcript path is shipped in Desktop and verified by the
`ISSUE-404` test and production Electron smoke matrix.
...
The shared contract is a discriminated union with these semantics (the live
types also carry the draft reference epoch and bounded request-path identity):
...
Tokens can be authorized only once at a time and are leased from turn start
until the native turn reaches a terminal state. Queued intents retain their
draft-bound token without leasing it; TTL expiry fails closed and restores the
intent to the visible Composer instead of silently dropping it. Tokens cannot
be replayed from another window, workspace or draft epoch.
...
- native turn completion/failure/interruption after successful acceptance;
...
Switching Agent or starting a new session releases draft and queued staged
tokens, clears their references, and rotates the draft epoch. PuppyOne does not
transfer a capability grant between native sessions or runtimes implicitly.
...
`npm run smoke:agent-references` runs real Electron with the production preload
and IPC handlers. Chromium assigns an actual filesystem-backed `File` to the
native file input, then the same OS grant is exercised through picker, drop and
paste routes. The smoke verifies dedupe, Explorer multi-entry resolution,
immutable snapshot consumption, active-lease protection and terminal cleanup.
It also loads the production Agent visual route at 420, 560 and 760 px in light
and dark themes and checks draft/committed chips, localized error visibility,
the accessible `+` menu and horizontal containment. DOM tests still cover the
full React drag-depth and typed Explorer-payload state machine.
