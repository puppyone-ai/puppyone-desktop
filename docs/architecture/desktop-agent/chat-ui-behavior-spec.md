...
- Static presentation declarations—including spacing, typography, color, height, transform,
  ...
  TSX must not contain literal inline style objects or own visual fallbacks.
...
- Horizontal outside margin is 12 px at every supported sidebar width, matching the Data sidebar's
  ...
  A ready empty session has no visible textarea placeholder; setup and recovery states may still
  use actionable placeholders. The textarea always retains an accessible name.
...
- While the cold-start loader is the only conversation state, the entire dock—including the
  Composer—is not mounted. It returns only after startup resolves or an actionable state replaces it.
